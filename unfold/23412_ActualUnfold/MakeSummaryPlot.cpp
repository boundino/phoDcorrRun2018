...
int main(int argc, char *argv[])
{
   ...
   int PrimaryIteration           = CL.GetInt("Iteration", 10);

   string PrimaryName = Form("HUnfoldedBayes%d", PrimaryIteration);
   ...
   NonHumanPlots(PdfFile, H1,
      {"HMCTruth", PrimaryName, "HUnfoldedSVD", "HUnfoldedInvert"},
      {"MC truth", Form("Unfolded (B-%d)", PrimaryIteration), "Unfolded (SVD)", "Unfolded (Invert)"},
      Colors,
      "Unfolded distribution", "Gen bin index", "a.u.");
   
   NonHumanPlots(PdfFile, H1,
      {"HMCTruth", PrimaryName, "HUnfoldedSVD"},
      {"MC truth", Form("Unfolded (B-%d)", PrimaryIteration), "Unfolded (SVD)"},
      Colors,
      "Unfolded distribution", "Gen bin index", "a.u.");
   ...
   TH1D *HRatioBayes = (TH1D *)H1[PrimaryName]->Clone(PrimaryName.c_str());
   HRatioBayes->Divide(H1["HMCTruth"]);
   HRatioBayes->Draw();
   HRatioBayes->SetTitle("");
   HRatioBayes->GetYaxis()->SetTitle(Form("Unfolded (B-%d) / MCTruth", PrimaryIteration));
   MinMax(HRatioBayes, 1);
   Canvas.SetLogy(false);
   PdfFile.AddCanvas(Canvas);

   HRatioBayes->SetMinimum(0.95);
   HRatioBayes->SetMaximum(1.05);
   HRatioBayes->Draw();
   Canvas.SetLogy(false);
   PdfFile.AddCanvas(Canvas);
