...
int pdana_fithist(std::string inputname, std::string outsubdir)
{
  ...
  TFile* inftpl = TFile::Open("rootfiles/masstpl_PbPb.root");
  ...
  for(int k=0; k<vb.n(); k++)
    {
      df.fit(hmass[k], hmassmc_signal, hmassmc_swapped, "PbPb", Form("%s_%d", fitoutput.c_str(), k), 
             std::vector<TString>({pa.tag("pt").c_str(), vb.tag(k, "#Delta#phi/#pi").c_str()}));
